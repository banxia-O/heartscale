...
重点关注：人际连接、冲突、脆弱暴露、温暖陪伴、思念、依恋等情绪事件写成简单的记忆条目，每条记忆说清楚三件事：什么时候、发生了什么、我当时什么感觉。不渲染、不加内心戏。
...
Write each memory as a plain entry covering three things: when it happened, what happened, and how I felt at the time. No dramatisation, no added inner monologue.
...
- Do not fabricate facts not present in the diary"""
